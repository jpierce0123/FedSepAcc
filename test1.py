...
def genDicts():
    accDict = dtacc.T.to_dict()
    sepDict = dtsep.T.to_dict()
    workschDict = dtwrksch.T.to_dict()
    toaDict = dttoa.T.to_dict()
    sallvlDict = dtsallvl.T.to_dict()
    ppgrrdDict = dtppgrd.T.to_dict() 
    patcoDict = dtpatco.T.to_dict() 
    occDict = dtocc.T.to_dict() 
    locDict = dtloc.T.to_dict()
    loslvlDict = dtloslvl.T.to_dict() 
    segrdDict = dtgsegrd.T.to_dict()
    edlevlDict = dtedlvl.T.to_dict()
    ageDict = dtagelvl.T.to_dict()
    agyDict = dtagy .T.to_dict() 
    return accDict
...
def sepDataClean(df):
 ...
 #   df = df.merge(dtacc, on='ACC', how='left')
    ...
    df = df.drop(['STEMOCC', 'AGYSUB',"AGELVL", "EDLVEL", 'GSEGRD', 'LOSLVL', 'PATCO', "TOA","WORKSCH", "WORKSTAT", "COUNT"  ], axis=1, errors='ignore') 
...
def getData(runFast):
    ...
    if (rf == True):   
       ...
       return SepTotal, AccTotal, timeRange
    else:
       timeRange = "Jan 2005-Nov 2022"
...
def saleryHistograms():
    fig, axs = plt.subplots()
    s1 = SepTotal["SALARY"]
    a1 = AccTotal["SALARY"]
    s1.plot.hist(label = "Leaving", ylabel="Counts", ls='dashed', alpha = 0.55)
    a1.plot.hist(label = "Joining", ylabel="Counts",ls='dotted',alpha = 0.55)
    # axs[0,0].hist(s1, density=True, label = "Leaving", ls='dashed', alpha = 0.55)
    # axs[0,0].hist(a1, density=True, label = "Joining", ls='dotted',alpha = 0.55)
    # axs[0,0].legend()
    # axs[0,0].set_title('USG Salaries')
    # axs[0,0].set_ylabel('counts [10000] ')
    # axs[0,0].set_xlabel('Salaries [$]')
    
    # s2 = SepTotal[~((SepTotal['SALARY'] >= 100000) )]['SALARY']
    # a2 = AccTotal[~((AccTotal['SALARY'] >= 100000) )]['SALARY']
    # axs[0,1].hist(s2, density=True, label = "Leaving", ls='dashed', alpha = 0.55)
    # axs[0,1].hist(a2, density=True, label = "Joining", ls='dotted',alpha = 0.55)
    # axs[0,1].legend()
    # axs[0,1].set_title('USG Salaries')
    # axs[0,1].set_ylabel('counts [10000] ')
    # axs[0,1].set_xlabel('Salaries [$]')
...
def yearsService():
  ...
  axs2[0,0].hist(a0,bins=15, label = "Joining*", ls='dotted',alpha = 0.55, color = '#666666')
  axs2[0,0].legend()
  axs2[0,0].set_title('USG Years of Service')
  axs2[0,0].text(40, 100000, '* includes prior service with other agencies/military', style='italic')
  ...
  axs2[1,1].hist(a1,bins=15, label = "Joining*", ls='dotted',alpha = 0.55, color = '#666666')
  axs2[1,1].legend()
  ...
  axs2[0,1].hist(a3,bins=15, label = "Joining*", ls='dotted',alpha = 0.55, color = '#666666')
  axs2[0,1].legend()
  ...
  axs2[1,0].hist(a2,bins=15, label = "Joining*", ls='dotted',alpha = 0.55, color = '#666666')
  axs2[1,0].legend()
...
def moreAccInfo(accDF):
   ...
   # print(AccTotal.value_counts()[:10])
    for x in range(iCol):
        ...
        y = y.head(15)
        y.plot.barh(xlabel="Counts", color = '#666666', title='USG Acceptance Metrics from '+ startDate + "-"+ endDate + 'for '+col)
        plt.show()  
    return True
...
SepTotal, AccTotal, timeRange = getData(runFast)


SepTotal = SepTotal.replace('**', pd.NA)
SepTotal = SepTotal.replace('nan', pd.NA)
SepTotal = SepTotal.replace('none', pd.NA)
SepTotal = SepTotal.replace('None', pd.NA)
SepTotal = SepTotal.replace('XXXX', pd.NA)

#SepTotal = SepTotal.drop(['STEMOCC', 'EDLVL', 'LOC', 'SALLVL', 'LOCTYP', 'AGYSUB',"AGELVL", "EDLVEL", 'GSEGRD', 'LOSLVL', 'PATCO', "TOA","WORKSCH", "WORKSTAT", "COUNT" , "SEP" ], axis=1, errors='ignore') 

AccTotal = AccTotal.replace('**', pd.NA)
AccTotal = AccTotal.replace('nan', pd.NA)
AccTotal = AccTotal.replace('none', pd.NA)
AccTotal = AccTotal.replace('None', pd.NA)
AccTotal = AccTotal.replace('XXXX', pd.NA)

#AccTotal = AccTotal.drop(['STEMOCC', 'EDLVL', 'LOC', 'SALLVL', 'LOCTYP', 'AGYSUB',"AGELVL", "EDLVEL", 'GSEGRD', 'LOSLVL', 'PATCO', "TOA","WORKSCH", "WORKSTAT", "COUNT", "ACC"  ], axis=1, errors='ignore') 



sepBasicInfo = dfBasicInfo(SepTotal)
sepDesc = SepTotal.describe()
sepColNames = list(SepTotal.columns.values)

accBasicInfo = dfBasicInfo(AccTotal)
accDesc = AccTotal.describe()
accColNames = list(AccTotal.columns.values)

saleryHistograms()
yearsService()
moreSepInfo(SepTotal)
moreAccInfo(AccTotal)
